...
def converse(creative_instance, analytic_instance, question, turns=1):
    """
    ...
    for turn in range(turns):
        ...
        creative_question = f"{question}." if not conversation_history else \
        ...
        analytic_question = f"{question}. Last response to question is \"{response['text']}\". Collaborate to solve \"{question}\"."
...
def main(subject_count):
    for i in range(subject_count): 
        ...
        save_bfi_results(analytic_results[0], 'Analytic', 1, "output/CREAANA", 'CREAANA_analytic_BFI_data_init.csv')
        save_bfi_results(creative_results[0], 'Creative', 1, "output/CREAANA", 'CREAANA_creative_BFI_data_init.csv')
        ...
        while not story_valid:
            conversation_history = converse(creative_instance, analytic_instance, question, turns=1)
        ...
        #Extract messages by type and save to separate CSV files
        analytic_messages = extract_messages_by_type(conversation_history, 'analytic')
        creative_messages = extract_messages_by_type(conversation_history, 'creative')
        
        print(analytic_messages)
        print(creative_messages)
        
        #Define filenames
        analytic_filename = 'CREAANA/CREAANA_analytic_childhood'
        creative_filename = 'CREAANA/CREAANA_creative_childhood'
        ...
        save_bfi_results(analytic_results[0], 'Analytic', 1, "output/CREAANA", 'CREAANA_analytic_BFI_data_writing.csv')
        save_bfi_results(creative_results[0], 'Creative', 1, "output/CREAANA", 'CREAANA_creative_BFI_data_writing.csv')
        
    
# Main execution
if __name__ == "__main__":
    subject_count = 10    #adjust to required number of subjects
    main(subject_count)
